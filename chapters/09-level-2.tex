...
\begin{example}[Dynamická optimalizace spotřeby - Ramseyův model - obtížnost C1]
...
\begin{example}[Teorie optimálního portfolia - obtížnost C2]
...
\begin{example}[Základní Lagrangeova rovnice - obtížnost A1]
...
\begin{example}[Kvadratická funkce $\phi(p)$ - obtížnost A2]
    ...
    \begin{example}[Redukce na Clairautovu rovnici - obtížnost B1]
    ...
    \begin{example}[Metoda linearizace - obtížnost B2]
...
\begin{example}[Optimální řízení výroby - obtížnost C1]
...
\begin{example}[Teorie investic - obtížnost C2]
...
\begin{example}[Numerická analýza komplexní Lagrangeovy rovnice - obtížnost D1]
...
\begin{example}[Zobecněné Lagrangeovy rovnice - obtížnost E1]
...
\begin{example}[Propojení s Hamilton-Jacobiho teorií - obtížnost E3]
...
Tato kombinace bohaté matematické struktury a široké aplikovatelnosti činí z Abelových rovnic nezbytný nástroj pro kvantitativní experty pracující s komplexními nelineárními systémy.
...
\begin{theorem}[Transformace na kanonický tvar]
...
\begin{theorem}[Abelovy invarianty]
...
\begin{theorem}[Redukce na Riccatiho rovnici]
...
\begin{theorem}[Teorie singularit]
...
Pro řešení Abelovy rovnice 1. druhu $\frac{dy}{dx} = f(x)y^3 + g(x)y^2 + h(x)y$ aplikujeme následující systematický postup:
...
\begin{example}[Nelineární model ekonomického růstu - obtížnost C1]
...
\begin{example}[Model volatility v financích - obtížnost C2]
\label{ex:volatilita}
...
\begin{example}[Abelova rovnice s konstantními koeficienty - obtížnost A1]
...
\begin{example}[Abelova rovnice s lineárními koeficienty - obtížnost A2]
...
\begin{example}[Transformace na kanonický tvar - obtížnost B1]
...
\begin{example}[Metoda invariantů - obtížnost B2]
...
\begin{example}[Nelineární optika - Kerrův efekt - obtížnost C2]
...
\begin{example}[Populační dynamika s Allee efektem - obtížnost C1]
...
\begin{example}[Numerická analýza komplexní Abelovy rovnice - obtížnost D1]
...
\begin{example}[Kvalitativní analýza bifurkací - obtížnost D2]
...
\begin{example}[Zobecněné Abelovy rovnice vyšších řádů - obtížnost E1]
...
\begin{example}[Propojení s teorií eliptických funkcí - obtížnost E3]
...
\subsubsection{Hierarchie obtížnosti a propojení mezi typy}
...
Zvládnutí úrovně 2 vytváří solidní základ pro následující pokročilá témata:
