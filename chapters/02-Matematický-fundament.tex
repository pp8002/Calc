...
\draw[blue, thick, domain=0:3.5, samples=100] plot (\x, {sin(2*\x r) + 0.5*sin(4*\x r) + 0.3*sin(6*\x r)});
\node at (2,1.8) {Složená funkce};
\draw[red, thick, domain=0:3.5, samples=100] plot (\x, {sin(2*\x r)});
\node at (1.2,1.2) [red] {Základní harmonická};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[scale=0.8]
\draw[fill=blue!20] (0,0) ellipse (1.5 and 1);
\node at (0,0) {Konvexní množina};
\draw[red, thick] (2,2) -- (2,-1);
\node[red] at (2.5,0.5) {Nadrovina};
\fill (3,0.5) circle (2pt) node[right] {Bod};
...
\draw[->] (0,0) -- (4,0) node[right] {Regularita};
\draw[->] (0,0) -- (0,3) node[above] {Prostor};
\node at (1,0.5) {$C[a,b]$};
\node at (2,1.2) {$L^p(a,b)$};
\node at (3,2.5) {$W^{1,p}(a,b)$};
\draw[->, thick] (1,0.6) -- (2,1.1);
\draw[->, thick] (2,1.3) -- (3,2.4);
\node[rotate=45] at (1.5,0.8) {kontinuální};
\node[rotate=63] at (2.5,1.8) {kompaktní};
\draw[<->, blue, thick] (1,2.8) -- (3.5,2.8) node[above, midway] {Funkce v $W^{1,2}(a,b)$: přirozený prostor pro slabá řešení ODR 2. řádu};
\end{tikzpicture}
